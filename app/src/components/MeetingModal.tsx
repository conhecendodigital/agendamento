...
export const MeetingModal: React.FC<MeetingModalProps> = ({
    ...
    const addParticipant = () => {
        ...
        setFormData(p => ({ ...p, participants: [...p.participants, email] }));
        setParticipantInput('');
        setErrors(p => ({ ...p, participant: undefined }));
    };

    const removeParticipant = (email: string) => {
        setFormData(p => ({ ...p, participants: p.participants.filter(e => e !== email) }));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            await onSave({ ...formData, status: editMeeting?.status || 'scheduled' });
            onClose();
        } catch (error) {
                        ...
                        {formData.participants.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-3">
                                {formData.participants.map(email => (
                                    <div
                                        key={email}
                                        className="flex items-center gap-2 bg-[#222] border border-white/5 px-3 py-1.5 rounded-full text-xs text-gray-300"
                                    >
                                        <span>{email}</span>
                                        <button
                                            type="button"
